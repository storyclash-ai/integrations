const DataFlowIllustration=()=>(
  <svg viewBox="0 0 400 320" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-auto">
    <rect x="40" y="40" width="120" height="80" rx="8" stroke="#00A1E0" strokeWidth="2" fill="rgba(0,161,224,0.05)"/>
    ...
    <text x="200" y="245" fontSize="12" fontWeight="500" fill="#00A1E0" textAnchor="middle">CRM</text>
    <circle cx="300" cy="180" r="30" stroke="#00A1E0" strokeWidth="2" fill="rgba(0,161,224,0.08)"/>
    <text x="300" y="185" fontSize="12" fontWeight="500" fill="#00A1E0" textAnchor="middle">Revenue</text>
...
export default function SalesforceWhyStoryclashSection(){
  const benefits=[{icon:SimplifyIcon,title:'Enterprise CRM Integration',description:'Storyclash syncs creator data to Salesforce using standard objects and custom fields. Manage influencers like enterprise accounts with complete CRM visibility.'},{icon:EnterpriseIcon,title:'Revenue Attribution',description:'Track creator impact on pipeline and closed-won deals. Build Salesforce reports showing influencer contribution to revenue and customer acquisition.'},{icon:ROIIcon,title:'Workflow Automation',description:'Trigger Salesforce automation based on creator performance. Automate follow-ups, notifications, and reporting as campaigns progress through stages.'}];
  return (
    <section className="py-20 px-6 bg-white border-t border-b" style={{borderColor:'#E5EAEC'}}>
      <div className="max-w-7xl mx-auto">
        <h2 className="text-4xl lg:text-5xl font-black mb-16 text-center tracking-tight" style={{color:'#00A1E0'}}>Why Storyclash for Salesforce?</h2>
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="order-2 lg:order-1"><div className="bg-gradient-to-br from-blue-50/50 to-transparent rounded-2xl p-8 lg:p-12"><DataFlowIllustration/></div></div>
          <div className="order-1 lg:order-2 space-y-8">
            {benefits.map((b,i)=>{const Icon=b.icon;return <div key={i} className="flex gap-5 group" style={{animation:`fadeInUp 0.6s ease-out ${i*0.15}s both`}}><div className="flex-shrink-0 w-12 h-12 flex items-center justify-center rounded-lg transition-all duration-300 group-hover:scale-110" style={{color:'#00A1E0'}}><Icon/></div><div><h3 className="text-lg font-bold text-gray-900 mb-2 tracking-tight">{b.title}</h3><p className="text-gray-600 leading-relaxed text-[15px]">{b.description}</p></div></div>})}
          </div>
        </div>
      </div>
